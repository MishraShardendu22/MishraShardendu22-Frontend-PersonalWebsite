...
export default function VolunteerPageContent() {
  ...
  const [selectedOrganisation, setSelectedOrganisation] = useState<string>('__all__')
  ...
  const allOrganisations = Array.from(new Set(experiences.map((e) => e.organisation)))
  const allYears = Array.from(
    new Set(
      experiences.flatMap((e) => 
        e.volunteer_time_line.map((timeline) => 
          timeline.start_date ? new Date(timeline.start_date).getFullYear().toString() : ''
        )
      )
    )
  ).filter(Boolean)
  ...
  const filteredExperiences = experiences.filter((experience) => {
    ...
    const matchesOrganisation =
      selectedOrganisation !== '__all__' ? experience.organisation === selectedOrganisation : true
    const matchesYear =
      selectedYear !== '__all__'
        ? experience.volunteer_time_line.some((timeline) =>
            timeline.start_date ? new Date(timeline.start_date).getFullYear().toString() === selectedYear : false
          )
        : true
    const matchesSearch =
      searchTerm === '' ||
      experience.volunteer_time_line.some((timeline) => 
        timeline.position.toLowerCase().includes(searchTerm.toLowerCase())
      ) ||
      experience.organisation.toLowerCase().includes(searchTerm.toLowerCase()) ||
      experience.description.toLowerCase().includes(searchTerm.toLowerCase())

    return matchesTech && matchesOrganisation && matchesYear && matchesSearch
  })
  ...
  const transformedExperiences = currentExperiences.map((experience) => {
    ...
    return {
      title: latestTimeline?.position || 'Volunteer',
  ...
  useEffect(() => {
    const fetchExperiences = async () => {
      try {
        const response = await volunteerExperiencesAPI.getAllVolunteerExperiences()
        ...
        <div className="container mx-auto px-6 py-4 max-w-full">
          <div className="flex items-center justify-between gap-8">
            ...
            <div className="flex items-center gap-6 flex-shrink-0">
              ...
              <div className="flex items-center gap-4">
                <div className="flex items-center gap-2">
                  ...
                  <h1 className="text-2xl font-heading font-bold bg-gradient-to-r from-pink-600 via-rose-600 to-pink-700 bg-clip-text text-transparent whitespace-nowrap">
                ...
                <div className="hidden lg:flex items-center gap-3 text-sm">
                  <div className="flex items-center gap-1">
                    <span className="font-semibold text-pink-600">{experiences.length}</span>
                    <span className="text-muted-foreground">Total</span>
                  </div>
                  <div className="w-px h-4 bg-border" />
                  <div className="flex items-center gap-1">
                    <span className="font-semibold text-pink-600">{currentPage}</span>
            ...
            <div className="flex-1 max-w-md mx-8">
              <Input
                type="text"
                placeholder="Search volunteer experiences..."
                value={searchTerm}
                onChange={(e) => handleSearchChange(e.target.value)}
                className="h-10 w-full border-2 border-border/50 hover:border-pink-500/50 focus:border-pink-500 transition-colors bg-background/50"
              />
            </div>
            ...
            {(selectedTech !== '__all__' ||
              ...
              <Button
                ...
                onClick={() => {
                  ...
                  setSelectedOrganisation('__all__')
      ...
      <div className="flex-1">
        <div className="container mx-auto px-6 py-8 max-w-full">
          ...
          <div className="flex items-center justify-between pt-6 border-t border-border/30 text-sm text-muted-foreground">
            ...
            <p className="text-xs">Community service and volunteer contributions</p>
          </div>
        </div>
      </div>
    </div>
