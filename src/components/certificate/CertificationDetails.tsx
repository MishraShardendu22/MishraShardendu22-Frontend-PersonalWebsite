    ...
    <div className="relative group">
      <div className="flex items-center justify-between bg-gray-800 px-4 py-2 rounded-t-lg">
        <div className="flex items-center gap-2">
          ...
          <span className="text-sm text-gray-400 font-medium">{language || 'code'}</span>
        </div>
        <Button
          variant="ghost"
          size="sm"
...
const CustomListItem = ({ children }: { children: React.ReactNode }) => (
  <li className="relative flex items-start gap-3 p-2 rounded-md hover:bg-muted/30 transition-colors">
  ...
  const customRenderers = {
    code: ({ node, inline, className, children, ...props }: any) => {
      ...
      return !inline && match ? (
        ...
        <code className="px-2 py-1 bg-muted rounded text-sm font-mono border" {...props}>
          {children}
        </code>
      )
    },
      ...
      <Card className="border border-border/50 bg-card/50 backdrop-blur-sm">
        ...
        >
          <div className="flex-grow">
            <div className="prose prose-lg max-w-none dark:prose-invert prose-headings:text-foreground prose-p:text-foreground prose-strong:text-foreground prose-code:text-foreground">
            ...
            <div className="mt-auto pt-8 space-y-6">
              ...
              <div className="bg-gradient-to-r from-muted/20 via-muted/30 to-muted/20 rounded-lg p-6 space-y-4 border border-border/30">
                ...
                <div className="grid grid-cols-2 gap-4 text-sm">
                  <div className="text-center p-4 bg-background/50 rounded-lg border border-border/20 hover:border-primary/30 transition-colors cursor-pointer">
      ...
      <div>
        <Card className="border border-border/50 bg-card/50 backdrop-blur-sm">
          <CardHeader className="pb-4">
            ...
            <CardDescription>
              Technical skills gained through this certification journey
            </CardDescription>
          </CardHeader>
          <CardContent>
